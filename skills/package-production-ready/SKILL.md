...
Scan all source files, docs, and commit messages for AI-generated tells:

- **Writing**: em-dashes (don't just replace with "--" though, rewrite like a human would), filler ("it's worth noting", "be sure to", "note that", "this ensures"), hedging ("might potentially"), aspirational adjectives ("robust", "comprehensive", "seamless", "elegant"), "This function/class provides..." openings, passive voice overuse
- **Code**: unnecessary docstrings on obvious one-liners, over-commenting self-explanatory code, type annotations/error handling/config options nobody asked for, features for hypothetical edge cases
- **Commits**: Co-authored-by trailers, "Generated by" or "Created with" comments
- **Structure**: over-formatted where a sentence would do, excessive bolding, suspiciously uniform bullet point grammar
